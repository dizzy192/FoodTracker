...
async def handle_food(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.info(f"Gemini result: {data}")
        ...
        if not data:
            msg = await update.message.reply_text("Не удалось распознать. Напиши иначе, например: съел шаурму и колу")
            await save_last_bot_message(user_id, msg.message_id)
            return
        ...
        logger.error(f"handle_food CRASH: {e}", exc_info=True)
        try:
